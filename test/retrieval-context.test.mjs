...
test('recall finds relevant memories with score breakdown', () => {
  ...
  const results = recall('how does the nextool deploy work', { silent: true });
...
test('quarantined content is never recalled', () => {
  ...
  const results = recall('zebras', { silent: true });
  assert.equal(results.filter((r) => r.id === q.memory.id).length, 0);
});

test('disputed memories are returned flagged', () => {
  ...
  const results = recall('indentation python', { silent: true });
  const flagged = results.filter((r) => r.disputed);
  assert.ok(flagged.length >= 2);
});

test('recall bumps access counts and logs a ledger event when not silent', () => {
  ...
  const results = recall('staging server port');
  assert.ok(results.length >= 1);
  const after = queryEvents({ eventType: 'memory.recalled' }).length;
  assert.equal(after, before + 1);
});

test('context capsule respects budget, excludes private, carries reasons and a receipt', () => {
  ...
  const capsule = compileContext('prepare the npm publish of soul v2', { tokenBudget: 800 });
...
test('tiny budget excludes memories and reports it', () => {
  for (let i = 0; i < 10; i++) {
  ...
  const capsule = compileContext('elephant migration project', { tokenBudget: 200 });
